...
export const PhotoStackCard = React.forwardRef(({ className, images, category, title, subtitle, logo, website, pills, isActive, onDescriptionClick, onPillClick, ...props }, ref) => {
	...
	return (
		<motion.div
			ref={ref}
			className={cn("group relative flex w-80 cursor-pointer flex-col justify-start rounded-xl bg-card p-6 shadow-xl showcase-card", pills ? "h-96" : "h-72", "transition-colors duration-300 ease-in-out hover:bg-card/90", className)}
